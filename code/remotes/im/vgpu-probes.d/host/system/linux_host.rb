...
class LinuxHost
    ...
    attr_accessor :cpu, :gpu, :memory, :net
    ...
    def initialize
        ...
        #  GPU
        ######
        nvidia-smi = `nvidia-smi vgpu -l`
        @gpu[:used] = nvidia-smi.count("\n") - 1
    ...
    def self.usage(hypervisor)
        ...
        print_info('FREEGPU', linux.gpu[:free])
        print_info('USEDGPU', linux.gpu[:used])
    ...
    def self.config(hypervisor)
        ...
        print_info('TOTALGPU', linux.gpu[:total])
